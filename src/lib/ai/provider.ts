...
const mechanicsMutationSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("update_character_progression_track"),
...
function formatRouterContextForModel(context: TurnRouterContext) {
  ...
  return {
    ...
    authoritativeState: {
      characterProgression: context.progression ?? null,
...
function buildTurnSystemPrompt(turnMode: TurnMode) {
  return turnMode === "observe"
    ? [
        ...
        "Use update_character_state for track-only conditions like disguised, poisoned, or exhausted; use update_character_progression_track only for defined persistent progression tracks shown in context.",
...
function buildTurnRouterSystemPrompt(correctionNotes?: string | null) {
  const base = [
    ...
    "economy_light covers incidental spending, bespoke haggling, bartering, discounts, negotiated deals over ungrounded goods or services, and self-directed character-state or character-progression changes.",
  ...
  async generateWorldModule(input: {
    ...
    try {
      ...
      const baseWorldModule: GeneratedWorldModule = {
        ...
        tone: worldBible.tone,
        setting: worldBible.setting,
        progressionFramework: input.previousDraft?.progressionFramework,
        locations: worldSpine.locations.map((location) => ({
          id: idMaps.locations[location.key],
          name: location.name,
          type: location.type,
